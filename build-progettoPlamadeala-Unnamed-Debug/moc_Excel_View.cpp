/****************************************************************************
** Meta object code from reading C++ file 'Excel_View.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.9.5)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../progettoPlamadeala/excel_View/Excel_View.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'Excel_View.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.9.5. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
QT_WARNING_PUSH
QT_WARNING_DISABLE_DEPRECATED
struct qt_meta_stringdata_Excel_View_t {
    QByteArrayData data[12];
    char stringdata0[79];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_Excel_View_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_Excel_View_t qt_meta_stringdata_Excel_View = {
    {
QT_MOC_LITERAL(0, 0, 10), // "Excel_View"
QT_MOC_LITERAL(1, 11, 8), // "NewShape"
QT_MOC_LITERAL(2, 20, 0), // ""
QT_MOC_LITERAL(3, 21, 6), // "QSize*"
QT_MOC_LITERAL(4, 28, 1), // "q"
QT_MOC_LITERAL(5, 30, 8), // "gotValue"
QT_MOC_LITERAL(6, 39, 1), // "x"
QT_MOC_LITERAL(7, 41, 1), // "y"
QT_MOC_LITERAL(8, 43, 8), // "newValue"
QT_MOC_LITERAL(9, 52, 10), // "rowChanged"
QT_MOC_LITERAL(10, 63, 1), // "s"
QT_MOC_LITERAL(11, 65, 13) // "columnChanged"

    },
    "Excel_View\0NewShape\0\0QSize*\0q\0gotValue\0"
    "x\0y\0newValue\0rowChanged\0s\0columnChanged"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_Excel_View[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       4,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: name, argc, parameters, tag, flags
       1,    1,   34,    2, 0x0a /* Public */,
       5,    3,   37,    2, 0x0a /* Public */,
       9,    2,   44,    2, 0x0a /* Public */,
      11,    2,   49,    2, 0x0a /* Public */,

 // slots: parameters
    QMetaType::Void, 0x80000000 | 3,    4,
    QMetaType::Void, QMetaType::Int, QMetaType::Int, QMetaType::Double,    6,    7,    8,
    QMetaType::Void, QMetaType::Int, QMetaType::QString,    7,   10,
    QMetaType::Void, QMetaType::Int, QMetaType::QString,    6,   10,

       0        // eod
};

void Excel_View::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Excel_View *_t = static_cast<Excel_View *>(_o);
        Q_UNUSED(_t)
        switch (_id) {
        case 0: _t->NewShape((*reinterpret_cast< QSize*(*)>(_a[1]))); break;
        case 1: _t->gotValue((*reinterpret_cast< int(*)>(_a[1])),(*reinterpret_cast< int(*)>(_a[2])),(*reinterpret_cast< double(*)>(_a[3]))); break;
        case 2: _t->rowChanged((*reinterpret_cast< int(*)>(_a[1])),(*reinterpret_cast< QString(*)>(_a[2]))); break;
        case 3: _t->columnChanged((*reinterpret_cast< int(*)>(_a[1])),(*reinterpret_cast< QString(*)>(_a[2]))); break;
        default: ;
        }
    }
}

const QMetaObject Excel_View::staticMetaObject = {
    { &ViewGraphInterface::staticMetaObject, qt_meta_stringdata_Excel_View.data,
      qt_meta_data_Excel_View,  qt_static_metacall, nullptr, nullptr}
};


const QMetaObject *Excel_View::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *Excel_View::qt_metacast(const char *_clname)
{
    if (!_clname) return nullptr;
    if (!strcmp(_clname, qt_meta_stringdata_Excel_View.stringdata0))
        return static_cast<void*>(this);
    return ViewGraphInterface::qt_metacast(_clname);
}

int Excel_View::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = ViewGraphInterface::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 4)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 4;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 4)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 4;
    }
    return _id;
}
QT_WARNING_POP
QT_END_MOC_NAMESPACE
